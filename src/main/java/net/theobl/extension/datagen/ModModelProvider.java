...
//        blockModels.createCraftingTableLike(ModBlocks.PALE_OAK_CRAFTING_TABLE.get(), Blocks.PALE_OAK_PLANKS, TextureMapping::craftingTable);
    ...
    public void createWall(Block wallBlock, Material texture, BlockModelGenerators blockModels) {
    ...
    public void createFire(Block fireBlock, BlockModelGenerators blockModels) {
        MultiVariant floorFireModels = blockModels.createFloorFireModels(fireBlock);
        MultiVariant sideFireModels = blockModels.createSideFireModels(fireBlock);
        blockModels.blockStateOutput
                .accept(
    ...
    public void createRedStoneLantern(Block lanternBlock, BlockModelGenerators blockModels) {
        ModelTemplate lanternTemplate = ModelTemplates.LANTERN;
        ModelTemplate hangingTemplate = ModelTemplates.HANGING_LANTERN;
        Identifier lantern = TexturedModel.LANTERN.create(lanternBlock, blockModels.modelOutput);
        Identifier hangingLantern = TexturedModel.HANGING_LANTERN.create(lanternBlock, blockModels.modelOutput);
        Identifier lanternOff = blockModels.createSuffixedVariant(lanternBlock, "_off", lanternTemplate,
                location -> new TextureMapping().put(TextureSlot.LANTERN, new Material(Identifier.parse("extension:block/redstone_lantern_off"))));
    ...
    private void createCauldrons(BlockModelGenerators blockModels) {
                        ...
                        createSimpleBlock(
                                ...
                                plainVariant(
                                        ModelTemplates.CAULDRON_FULL
                                                .create(ModBlocks.MILK_CAULDRON.get(), TextureMapping.cauldron(new Material(Identifier.fromNamespaceAndPath("neoforge", "block/milk_still"))), blockModels.modelOutput)
                                )
                        )
                );
        Identifier potionLevel1 = CAULDRON_CONTENT_LEVEL1
        ...
        Identifier potionLevel2 = CAULDRON_CONTENT_LEVEL2
                .create(Extension.asResource("cauldron_potion_level2").withPrefix("block/"),
                        new TextureMapping().put(TextureSlot.CONTENT, TextureMapping.getBlockTexture(Blocks.WATER, "_still")),
                        blockModels.modelOutput);
        Identifier potionLevel3 = CAULDRON_CONTENT_LEVEL3
                .create(Extension.asResource("cauldron_potion_level3").withPrefix("block/"),
                        new TextureMapping().put(TextureSlot.CONTENT, TextureMapping.getBlockTexture(Blocks.WATER, "_still")),
                        blockModels.modelOutput);
        Identifier potionFull = CAULDRON_CONTENT_FULL
